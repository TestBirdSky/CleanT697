...
class JunkFileRepository(
    ...
    // 用于跟踪已找到的文件路径，防止重复
    private val foundFiles = mutableSetOf<String>()
    ...
    suspend fun scanJunkFiles(listener: ScanProgressListener) = withContext(Dispatchers.IO) {
        ...
        try {
            for (rootDir in rootDirs) {
                ...
                scanDirectory(rootDir, listener, { junkFile ->
                    // 检查文件是否已经添加过，避免重复
                    if (!foundFiles.contains(junkFile.path)) {
                        foundFiles.add(junkFile.path)

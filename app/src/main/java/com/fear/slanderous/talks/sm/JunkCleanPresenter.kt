...
class JunkCleanPresenterImpl(
    ...
    override fun toggleFileSelection(file: JunkFile, category: JunkCategory) {
        ...
        if (categoryIndex != -1) {
            ...
            if (fileIndex != -1) {
                ...
                val allFilesSelected = categories[categoryIndex].files.all { it.isSelected }
    ...
    private fun addJunkFile(junkFile: JunkFile) {
        ...
        if (categoryIndex != -1) {
            // 检查文件是否已存在于列表中，避免重复添加
            val fileExists = categories[categoryIndex].files.any { it.path == junkFile.path }
            if (!fileExists) {
